...
void SelectDataStructure()
{
  ...
  while(dataStructure==0 || (dataStructure!=1 && dataStructure!=2))
  {
    dataStructure=0;
    std::cout << '\n';
    std::cout<<"1) Create a random Vector "<<std::endl;
    std::cout<<"2) Create a random List "<<std::endl;
...
void SelectSize(const uint& dataStructure, const uint& type)
{
  ...
  {
    size=0;
    std::cout<< ((dataStructure == 1) ? "Enter Vector(" : "List(")<< typeString <<") size:";
    std::cin>>size;
    std::cout << ((size < 0) ? "Size can't be a negative number." : "")<< '\n';
  }
  CreateStructure(dataStructure,type,size);
}
...
void CreateStructure(const unsigned int& dataStructure, const unsigned int& type, const unsigned int& size)
{
  if(dataStructure==1)
  {
    if(type==1)
