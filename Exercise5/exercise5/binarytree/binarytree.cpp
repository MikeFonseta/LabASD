...
{
  if(key.HasLeftChild()) { FoldInOrder(fun,par,acc,key.LeftChild()); }
  fun(key.Element(),par, acc);
  if(key.HasRightChild()){ FoldInOrder(fun,par,acc,key.RightChild());}
}
...
{
  if(size > 0) { FoldInOrder(fun,par,acc,Root()); }
}
...
BTPostOrderIterator<Data>& BTPostOrderIterator<Data>::operator++()
{
  if(!Terminated())
  {
    ...
    if(!stk.Empty())
    {
      ...
      typename BinaryTree<Data>::Node* hasLeft = 0;
      if(tmp->HasLeftChild()) { hasLeft = &tmp->LeftChild(); }
      if(hasLeft == current && tmp->HasRightChild())
      {
